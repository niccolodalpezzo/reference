...
const DEMO_CREDENTIALS = [
  { label: 'Professionista', email: 'marco@demo.it', role: 'member', color: 'bg-ndp-blue' },
  { label: 'Resp. di Zona', email: 'luca@demo.it', role: 'zone_manager', color: 'bg-ndp-gold-dark' },
];
...
function LoginForm() {
  ...
  useEffect(() => {
    if (!isLoading && user) {
      ...
      else if (user.role === 'member') router.replace('/professionisti/dashboard');
      else router.replace('/');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    setSubmitting(true);
    const demoUser = authLogin(email, password);
    if (!demoUser) {
      setError('Credenziali non valide. Usa le credenziali demo qui sotto.');
    } else {
      login(demoUser);
    }
    setSubmitting(false);
    ...
    <div className="min-h-screen bg-ndp-bg flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md">
          ...
          <div className="mb-6">
            ...
            <p className="text-ndp-muted text-sm">
              {from === 'zone_manager'
                ...
                : 'Scegli il tuo ruolo e accedi alla piattaforma.'}
            </p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full px-4 py-2.5 pr-10 border border-ndp-border rounded-xl text-sm text-ndp-text placeholder-ndp-muted focus:outline-none focus:border-ndp-blue/50 focus:ring-1 focus:ring-ndp-blue/20 transition-all"
                />
                <button
                  type="button"
            ...
            <div className="grid grid-cols-2 gap-3">
              {DEMO_CREDENTIALS.map((c) => (
                ...
                >
                  ...
                  <div className="opacity-75 text-[10px]">password: demo</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <p className="text-center text-xs text-ndp-muted mt-6">
          Questa è una demo per la presentazione BNI. Non inserire dati reali.
        </p>
      </div>
    </div>
  );
