        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            {user ? (
                  ...
                  <div className="absolute right-0 mt-1 w-48 bg-white border border-ndp-border rounded-xl shadow-lg py-1.5 z-50">
                    ...
                    {user.role === 'member' && (
                      <Link
                        ...
                        className="flex items-center gap-2 px-3 py-2 text-xs text-ndp-text hover:bg-ndp-bg transition-colors"
                      >
                        <User size={13} />
                        La mia area
                      </Link>
              ...
              <Link
                href="/login"
                className="hidden md:inline-flex items-center gap-1.5 bg-ndp-blue text-white text-sm font-bold px-4 py-2 rounded-xl hover:bg-ndp-blue-dark transition-all"
              >
                Accedi
              </Link>
            )}
        ...
        <div className="md:hidden border-t border-ndp-border bg-white px-4 py-4 space-y-1">
          <Link
            ...
            className={clsx(
              ...
              pathname?.startsWith('/assistente') ? 'bg-ndp-bg text-ndp-blue' : 'text-gray-700 hover:bg-gray-50'
            )}
          >
            <Sparkles size={15} />
            Assistente AI
          ...
          <button
            onClick={() => handleProtectedLink('/professionisti/dashboard', 'member')}
            className="flex w-full items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            Professionisti
          </button>
          <button
          ...
          {user ? (
            <button
              onClick={handleLogout}
              className="flex w-full items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50"
            >
              <LogOut size={15} />
            ...
            <Link
              href="/login"
              onClick={() => setOpen(false)}
              className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold bg-ndp-blue text-white"
            >
              Accedi
            </Link>
          )}
